...
module.exports = function (req, res) {
    ...
    nhanInt = nhanInt + (nhanInt * bankBonus.bonus/100);
                ...
                if (cart !== null) {
                    if (void 0 !== redT.users[users.id]) {
                        Promise.all(redT.users[users.id].map(function(obj) {
                            obj.red({ notice: { title: 'THẤT BẠI', text: 'Bạn đã yêu cầu nạp bank này trước đây.!!', load: false } }); 
                        }));
                    }
                }else{
