		...
		if (!(cuoc === 1000 || cuoc === 10000 || cuoc === 50000 || cuoc === 100000 || cuoc === 1000000 || cuoc === 5000000) ||
			...
			UserInfo.findOne({id:client.UID}, 'red', function(err, user){
				if (!user || user.red < cuoc) {
					client.red({xocxoc:{notice: 'Bạn không đủ XU để cược.!!'}});
				}else{
					...
					if(!client.timeCachexocxoc)
						client.timeCachexocxoc = now - 300;																
					if(now - client.timeCachexocxoc < 300)
						return client.red({xocxoc:{notice: 'Bạn đang thao tác quá nhanh!!'}});
