						...
						.then(result => {
							...
							let thuong = 0;
								...
								Caoboi_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.red({Caoboi:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.Caoboi.free, isFree:isFree, isBonus:client.Caoboi.bonusL, isNoHu:nohu, isBigWin:isBigWin, phien:small.id}, user:{red:user.red-tongCuoc+bet_win}});
