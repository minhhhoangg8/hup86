					...
					HU.findOne({game:'long', type:bet}, {}, function(err2, dataHu){
						...
						.then(result => {
							...
							LongLan_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
								client.red({longlan:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.LongLan.free, isFree:isFree, isBonus:client.LongLan.bonusL, isNoHu:nohu, isBigWin:isBigWin, phien:small.id}, user:{red:balans+bet_win}});
