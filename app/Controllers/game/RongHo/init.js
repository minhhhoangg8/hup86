	...
	this.timeInterval = setInterval(function(){
		...
		if (this.time < 30) {
			if (this.time < 0) {
						...
						if (bcXX.bot) {
							...
							UserInfo.find({type:true}, 'id name', function(err, blist){
								if (blist.length) {
									...
									.then(result => {
										let maxBot = (result.length*(Math.floor(Math.random()*(70-50+1))+50)/100)>>0;
				...
				let cc = Object.keys(this.clients).length+(200*(Math.floor(Math.random()*(70-50+1))+50)/100)>>0;
