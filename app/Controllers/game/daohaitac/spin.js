						...
						.then(result => {
							...
							if (red) {
								...
								Daohaitac_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.red({daohaitac:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.Daohaitac.free, isFree:isFree, isBonus:client.Daohaitac.bonusL, isNoHu:nohu, isBigWin:isBigWin, phien:small.id}, user:{red:user.red-tongCuoc+bet_win}});
							...
							UserInfo.updateOne({id:client.UID},{$inc:uInfo}).exec();
								...
								///	client.red({daohaitac: {user:{red: user.red *1 + bet_win}}});
								//});
