				...
				if (checkPhone) {
					OTP.findOne({'uid':checkPhone.uid, 'phone':checkPhone.phone}, {}, {sort:{'_id':-1}}, function(err, data){
						if (!data || ((new Date()-Date.parse(data.date))/1000) > 180 || data.active) {
							...
							bot.sendMessage(id, '*OTP*:  ' + otp + '', {parse_mode:'markdown', reply_markup:{resize_keyboard:true}});
							bot = null;
							id = null;
							checkPhone = null;
						}else{
							bot.sendMessage(id, 'OTP:  _' + data.code + '_', {parse_mode:'markdown', reply_markup:{resize_keyboard:true}});
							bot = null;
							checkPhone = null;
							id = null;
						}
