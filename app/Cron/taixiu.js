...
let botchatRun = function(){
	...
	let botChat = setInterval(function(){
			let _time = 1000 * parseFloat((Math.random() * (0.9 - 0.3) + 0.5).toFixed(4));//
			...
			Promise.all(list.map(function(obj){
				let action = new Promise((resolve, reject)=> {
					TaiXiu_User.findOne({uid:obj.uid}, function(error, data) {
						if (!!data) {
							let bet_thua = obj.bet-obj.betwin;
...
let playGame = function(){
	...
	//	bot.regbot();
		...
		if (io.TaiXiu_time == 2) {
			// đầu tiên là random xúc xắc trước
			dice1 = Math.floor(Math.random() * 6) + 1;
			dice2 = Math.floor(Math.random() * 6) + 1;
			dice3 = Math.floor(Math.random() * 6) + 1;
			// kiểm tra xuc xac
			let totaldice = dice1+dice2+dice3;
			let taixiu =  true; // true tai false xiu
			if(totaldice <= 10){
				taixiu = false;
			}
			let lechTai = io.taixiuAdmin.taixiu.red_tai - io.taixiuAdmin.taixiu.red_xiu; // >0 tai <0 xiu
			if(QuyTaiXiu = 0){
				console.log("xxx11")
				// hut du quy
				if(lechTai = 0 && taixiu){
			...
			}else if(QuyTaiXiu = 0) { // quy nho hon hoac = 0 
				console.log("xxx22")
				if(lechTai = 0 && taixiu){
					dice1 = 7-dice1;
					dice2 = 7-dice2;
		...
		if (io.TaiXiu_time == 5) {
			...
			if (io.taixiu.taixiu.red_tai > io.taixiu.taixiu.red_xiu) {
				...
				home = {taixiu:{taixiu:{red_tai: io.taixiu.taixiu.red_tai, red_xiu: io.taixiu.taixiu.red_xiu},err: 'CHUẨN BỊ CÂN CỬA'}};
			}else{
		...
		if (io.TaiXiu_time <= 5) {
			...
			// Users
			let home;
			let red_tai = Math.abs(io.taixiu.taixiu.red_tai);
			let red_xiu = Math.abs(io.taixiu.taixiu.red_xiu);
			if (red_tai > red_xiu) {
				red_tai = red_xiu;
				home = {taixiu:{taixiu:{red_tai: red_tai, red_xiu: red_xiu}}};
			}else{
				red_xiu = red_tai;
				home = {taixiu:{taixiu:{red_tai: red_tai, red_xiu: red_xiu}}};
			}
		...
		if (io.TaiXiu_time <= 60) {
			if (io.TaiXiu_time < 0) {
				...
				if (!!taixiujs) {
					if(taixiujs.uid != ""){
						// neu co dat kq cho phien hien tai
						dice1 = taixiujs.dice1;
						dice2 = taixiujs.dice2;
						dice3 = taixiujs.dice3;
						// quy se khong anh huong
						taixiujs.uid = "";
						taixiujs.rights = 2;
				...
				if (!!taixiucf && taixiucf.bot && !!io.listBot && io.listBot.length > 0) {
					...
					let maxBot = (botList.length*250/100)>>0;
