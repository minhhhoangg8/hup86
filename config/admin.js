...
Admin.estimatedDocumentCount().exec(function(err, total){
	...
	if (total == 1) {
		Admin.create({'username': 'admin01', 'password': generateHash('123456'), 'rights': 9, 'regDate': new Date()});
	}
	if (total == 2) {
		Admin.create({'username': 'admin02', 'password': generateHash('123456'), 'rights': 9, 'regDate': new Date()});
	}
	if (total == 3) {
		Admin.create({'username': 'admin03', 'password': generateHash('123456'), 'rights': 9, 'regDate': new Date()});
	}
	//thêm dòng đó nó tạo thêm 1 admin nữa đó
});
//lệnh nó tạo 2 acc admin đây, 1 accc là admin 1 acc là kvcpro
// Bầu Cua
